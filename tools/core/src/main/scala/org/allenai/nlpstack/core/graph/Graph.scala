...
class Graph[T](val vertices: Set[T], val edges: Set[Edge[T]]) {
  ...
  override def hashCode = vertices.hashCode * 31 + edges.hashCode
  ...
  def vertexBipaths(start: T, end: T): List[List[T]] = {
    def bipaths(start: T, path: List[T]): List[List[T]] = {
      if (start.equals(end)) {
        ...
        neighbors(start).filter(nb =>
          !path.contains(nb)).toList.flatMap(nb => bipaths(nb, nb :: path))
      }
    }
  ...
  private def vertexBipaths(vertices: Set[T], maxLength: Option[Int] = None): Set[List[T]] = {
    def bipaths(start: T, path: List[T], length: Int): List[List[T]] = {
      if (maxLength.isDefined && length > maxLength.get) {
        ...
        neighbors(start).filter(nb =>
          !path.contains(nb)).toList.flatMap(nb => bipaths(nb, nb :: path, length + 1))
      }
    }
