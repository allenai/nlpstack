package org.allenai.aitk.graph.pattern

import org.allenai.aitk.graph.{ Graph, DirectedEdge, Bipath }
...
class Pattern[T](
  ...
  def this(edgeMatchers: List[EdgeMatcher[T]], nodeMatchers: List[NodeMatcher[T]]) = {
    this(Pattern.interleave(nodeMatchers, edgeMatchers).toList)
  }
...
object Pattern {
  private def interleave[T](x: Iterable[_ <: T], y: Iterable[_ <: T]): Seq[T] = {
    val xIt = x.iterator
    val yIt = y.iterator
