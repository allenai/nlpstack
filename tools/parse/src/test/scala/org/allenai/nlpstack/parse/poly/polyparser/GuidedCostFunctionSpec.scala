...
class GuidedCostFunctionSpec extends UnitSpec {
  ...
  "Calling GuidedCostFunction's apply" should "duplicate the steps needed to create parse1" in {
    ...
    val initialState: State = transitionSystem.initialState(parse1.sentence).get
    ...
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift)) map { state =>
        costFunction(state) } shouldBe Some(Map(ArcEagerInvertedLeftArc('det) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det))) map { state =>
        costFunction(state) } shouldBe Some(Map(ArcEagerShift -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det), ArcEagerShift)) map { state =>
        costFunction(state) } shouldBe Some(Map(ArcEagerLeftArc('nsubj) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det), ArcEagerShift, ArcEagerLeftArc('nsubj))) map { state =>
        costFunction(state) } shouldBe Some(Map(ArcEagerRightArc('root) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det), ArcEagerShift, ArcEagerLeftArc('nsubj), ArcEagerRightArc('root))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerInvertedRightArc('prep) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det), ArcEagerShift, ArcEagerLeftArc('nsubj),
        ArcEagerRightArc('root), ArcEagerInvertedRightArc('prep))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerRightArc('pobj) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerShift, ArcEagerInvertedLeftArc('det), ArcEagerShift, ArcEagerLeftArc('nsubj),
        ArcEagerRightArc('root), ArcEagerInvertedRightArc('prep), ArcEagerRightArc('pobj))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerReduce -> 0))
  }

  it should "duplicate the steps needed to create parse2" in {
    val transitionSystem = ArcEagerTransitionSystem()
    val initialState: State = transitionSystem.initialState(parse2.sentence).get
    ...
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerRightArc('a2b) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a), ArcEagerRightArc('a2b))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerShift -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a), ArcEagerRightArc('a2b), ArcEagerShift)) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerLeftArc('d2c) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a), ArcEagerRightArc('a2b), ArcEagerShift, ArcEagerLeftArc('d2c))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerReduce -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a), ArcEagerRightArc('a2b), ArcEagerShift, ArcEagerLeftArc('d2c), ArcEagerReduce)) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerRightArc('a2d) -> 0))
    StateTransition.applyTransitionSequence(initialState,
      List(ArcEagerRightArc('n2a), ArcEagerRightArc('a2b), ArcEagerShift, ArcEagerLeftArc('d2c),
        ArcEagerReduce, ArcEagerRightArc('a2d))) map { state =>
      costFunction(state) } shouldBe Some(Map(ArcEagerReduce -> 0))
  }
