...
private case class ParseCacheCommandLine(fallbackParserFilename: String = "",
  ...
  def main(args: Array[String]) {
    val optionParser = new OptionParser[ParseCacheCommandLine]("ParseCache") {
      opt[String]('f', "fallbackParser") required() valueName ("<file>") action { (x, c) =>
        c.copy(fallbackParserFilename = x)} text ("the file containing the fallback" +
        " parser configuration")
      opt[String]('g', "goldfile") required() valueName ("<file>") action { (x, c) =>
        c.copy(goldParseFilenames = x)} text ("the file containing the gold parses")
      opt[String]('o', "outputfile") required() valueName ("<file>") action { (x, c) =>
        c.copy(outputFilename = x)} text ("where to write the CachingParser")
...
case class ParseCache(cachedParses: Seq[(String, PolytreeParse)],
  fallbackParser: TransitionParser) extends TransitionParser {
